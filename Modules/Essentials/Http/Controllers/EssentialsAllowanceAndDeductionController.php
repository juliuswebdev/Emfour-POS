...
class EssentialsAllowanceAndDeductionController extends Controller
{
    ...
    public function index()
    {
        ...
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'hris_module') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module'))) {
    ...
    public function create()
    {
        ...
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'hris_module') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'essentials_module')) || ! auth()->user()->can('essentials.add_allowance_and_deduction')) {
            abort(403, 'Unauthorized action.');
        }
