...
<div class="row">
    <div class="col-sm-9">
        {!! Form::open(['url' => action([\App\Http\Controllers\Restaurant\BookingController::class, 'update'], [$booking->id]), 'method' => 'PUT', 'id' => 'checkout_booking_form', 'class' => 'check_booking_form' ]) !!}
            <div class="input-group">
                <input type="hidden" name="booking_status" value="checkout">
                <input type="hidden" name="booking_product_ids" value="{{ $booking->product_id }}">
                <input type="hidden" name="booking_ref_no" value="{{ $booking->ref_no }}">
                <input type="hidden" name="booking_correspondent_id" value="{{ $booking->correspondent_id }}">
                <input type="hidden" name="booking_customer" value="{{ $booking->contact_id }}">
                <!-- /btn-group -->
                <div class="input-group-btn">
                    <button type="submit" class="btn btn-primary">@lang('restaurant.checkout')</button>
                </div>
            </div>
        {!! Form::close() !!}
    </div>
