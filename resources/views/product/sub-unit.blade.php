@extends('layouts.app')
@section('title', __('lang_v1.sub_unit_inventory'))

@section('content')

<!-- Content Header (Page header) -->
<section class="content-header">
    <h1>@lang('lang_v1.sub_unit_inventory')</h1>
</section>
...
<div class="row">
    ...
    @component('components.widget')
        <div id="product_sub_unit_inventory" style="display: none;"></div>
    @endcomponent
    </div>
</div>
   ...
   <script type="text/javascript">
        $(document).ready( function(){
            ...
            $('#product_id').select2({
                ...
                window.location.href = "{{url('/')}}/products/sub-unit-inventory/" + data.id
            });
        });

       function load_stock_history(variation_id, location_id) {
            $('#product_sub_unit_inventory').fadeOut();
            $.ajax({
                url: '/products/sub-unit-inventory/' + variation_id + "?location_id=" + location_id,
                dataType: 'html',
                success: function(result) {
                    $('#product_sub_unit_inventory')
                        .html(result)
                        .fadeIn();

                    __currency_convert_recursively($('#product_sub_unit_inventory'));
